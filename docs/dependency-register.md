...
| GitHub branch protection | non-bypassable CI gate on `main` (FND-04, FND-05) | pending/deferred | paulakintunde/AppFincWin | GitHub Free plan, private repo — `PUT .../branches/main/protection` returns 403 ("Upgrade to GitHub Pro or make this repository public to enable this feature") | Phase 0 (FND-04/05 non-bypassable intent); user accepted CI-advisory-only for now | 2026-09-23 | CI itself is live and green (`checks`/`secret-scan`/`rls`, run 35890330292 on `aeaff09`) — only the required-status-check merge gate is blocked. Unblock: GitHub Pro ($4/mo) or Team, then re-run the `gh api -X PUT .../protection` call from 00-08-PLAN.md Task 2 verbatim. User declined making the repo public and declined GitHub rulesets as a substitute (00-08) |
